...
public:
    ...
    const std::array<size_t, 1+sigma> m_symbol_ends{};
    const std::array<size_t, 1+sigma> m_edge_max_ranks{};
  ...
  public:
    ...
    debruijn_graph_shifted(size_t in_k, const t_bit_vector_type & node_flags, const t_edge_vector_type & edges,
...
private:
    std::array<size_t, 1+sigma> _init_max_ranks(const t_edge_vector_type & edges) {
        std::array<size_t, 1+sigma> max_ranks;
...
public:
    static debruijn_graph_shifted load_from_packed_edges(std::istream & input, label_type alphabet=label_type{}, std::vector<size_t> * v=nullptr) {
        ...
        // read footer
        input.seekg(-(sigma+2) * sizeof(uint64_t), std::ios::end);
        std::array<size_t,1+sigma> counts{};
        uint64_t k = 0;
        ...
        std::vector<uint64_t> blocks(num_blocks,0);
        ...
        sdsl::int_vector<1> first(num_edges,0);
        // would be nice to fix wavelet trees so the constructor
        // can accept a int_vector<4> instead (which is all we need for DNA)
        sdsl::int_vector<8> edges(num_edges);

        bool prev_was_minus = false;
        for (size_t i = 0; i < num_edges; i++) {
            auto x = get_edge(blocks.begin(), i);
            first[i] = 1-std::get<1>(x); // convert 0s to 1s so we can have a sparse bit vector
            // For branchy graphs it might be better to change this and use RRR
            edges[i] = (std::get<0>(x) << 1) | !std::get<2>(x);
            if (v && std::get<0>(x) && !std::get<2>(x) && !prev_was_minus) {
                v->push_back(i);
                prev_was_minus = true;
            }
            else if (v && std::get<0>(x) && std::get<2>(x)) prev_was_minus = false;
        }
    ...
    std::vector<node_type> all_preds(const node_type & v) const {
        // node u -> v : edge i -> j
        size_t j = std::get<0>(v);
        symbol_type y = _symbol_access(j);
        if (y == 0) return std::vector<node_type>(0);
        ...
        std::vector<node_type> result(num_predecessors);
  ...
  ssize_t interval_node_outgoing(const node_type & u, symbol_type x) const {
    ...
    if (x == 0) return -1;
    //auto range = _node_range(u);
    size_t first = std::get<0>(u);
    size_t last  = std::get<1>(u);
    // Try both with and without a flag
    for (symbol_type c = _with_edge_flag(x,false); c <= _with_edge_flag(x, true); c++) {
...
public:
    ...
    node_type get_node(size_t v) const {
        ...
        return node_type(std::get<0>(r), std::get<1>(r));
    }

  // provided for DCC paper
  inline symbol_type lastchar(const node_type & v) const {
    return _symbol_access(std::get<0>(v));
  }
...
public:
   ...
   size_t my_node_select(size_t t, const boost::dynamic_bitset<> &node_flags) const
    ...
    size_t my_node_select2(const boost::dynamic_bitset<> &node_flags, std::vector<std::pair<signed long long, signed long long>> &selectcache,
                           symbol_type x, size_t t) const
    {
        if (std::get<0>(selectcache[x]) == -1) {
            size_t ret = m_node_select(t);
            selectcache[x] = std::make_pair(t, ret);
            return ret;
        } else {
            if ((size_t)std::get<0>(selectcache[x]) == t) {
    ...
    void get_column(const std::vector<size_t> &node_rank_cache, const boost::dynamic_bitset<> &node_flags, const std::vector<symbol_type> &edges, const std::vector<symbol_type> &oldcol, std::vector<symbol_type> &newcol) const
        {
            ...
            for (int i = 0; i < 5; i++)
                selectcache.push_back(std::make_pair(-1,-1));
...
//                if (my_new_edges_begin != new_edges_begin) std::cout << "b select( " << new_nth_node <<") diff =" <<  (signed long long)my_new_edges_begin - (signed long long)new_edges_begin << std::endl;
  ...
  size_t _last_edge_of_node(size_t v) const {
    ...
    std::pair<size_t, size_t> _node_range(size_t v) const {
        return std::make_pair(_first_edge_of_node(v), _last_edge_of_node(v));
    }
    ...
    boost::optional<node_type> index(InputIterator in) const {
        ...
        size_t first=0, last=0; //Initial value unimportant, just getting rid of warning
